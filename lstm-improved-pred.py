...
Y_sc.fit_transform(data_prices['Close'].values.reshape(-1, 1))
dump(Y_sc, open('scaler.pkl', 'wb'))
...
X_train, X_test, X_val, y_train, y_test, y_val = create_data_scaled(data_prices_scaled, n_future=1, n_past=40,
                                                                    train_test_split_percentage=0.9,
                                                                    validation_split_percentage=0)
# ------------------LSTM-----------------------
regressor = Sequential()
regressor.add(
    LSTM(units=16, return_sequences=False, activation='relu', input_shape=(X_train.shape[1], X_train.shape[2])))
...
history = regressor.fit(X_train, y_train, validation_split=0.3, epochs=32, batch_size=16, callbacks=[es])
print(history.history.keys())

regressor.save_weights('lstmi-x-2.json')

y_pred = regressor.predict(X_test)
y_pred = Y_sc.inverse_transform(y_pred)
y_test = Y_sc.inverse_transform(y_test)

np.savetxt("predtest-lstmi-1.csv", y_pred, delimiter=",")
np.savetxt("realtest-lstmi-1.csv", y_test, delimiter=",")
